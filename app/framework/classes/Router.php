...
class Router
{
    ...
    private function routerFound($routes)
    {
        ...
        if(!isset($routes[$this->request])){
            throw new \Exception("Route {$this->path} does nor exist");
        }
    ...
    private function controllerFound(string $controllerNamespace, string $controller, string $action)
    {
        if(!class_exists($controllerNamespace)){
            throw new Exception("O controller {$controller} does not exist");
        }
        if(!method_exists($controllerNamespace, $action)){
            throw new Exception("A action {$action} does not exist in controller {$controller}");
        }
    }
    ...
    public function execute() {
        ...
        $controllerNamespace = "app\\controllers○2\${$controller}";
